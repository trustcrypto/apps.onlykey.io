    ...
    function PGP_API() {
      ...
      async function u2fSignBuffer(slot, cipherText, mainCallback) {
        ...
        if (!ctaphid_response || ctaphid_response.error) {
          var chunk_error = (ctaphid_response && ctaphid_response.error) || "no response";
          console.warn("OnlyKey: packet " + packetnum + " did not reach the device:", chunk_error);
          imports.app.emit("ok-error");
          // Tell the page. Returning without this left the operation waiting
          // on a callback that never comes: the button kept saying "working"
          // and the device's reason - e.g. that stored-key use over FIDO2 is
          // off, which is the default - was only in the console.
          _api.emit("error", explainDeviceError(chunk_error));
          return;
        }
      ...
      function doPinTimer(seconds) {
        return new Promise(async function updateTimer(resolve, reject, secondsRemaining) {
          ...
          if (!_$status_is('pending_challenge') /* || _$status_is('waiting_ping')*/ ) {
            ...
            if (secondsRemaining > 1) {
              /* This used to read "You have N seconds to enter challenge code
               * X,Y,Z on OnlyKey", which is only true when the key is set to
               * challenge codes - and says the wrong thing, in the wrong
               * direction, when it is not.
               *
               * okcore_prime_user_confirmation() arms press mode at
               * CRYPTO_AUTH = 3 and never computes Challenge_button1/2/3, so
               * the first press of ANY button matches the USER_INPUT_PRESS
               * disjunct in OnlyKey.ino's button handler, reaches
               * CRYPTO_AUTH = 4 and runs the operation. Measured on hardware
               * 2026-09-21: the page said "enter challenge code 6,2,5", one
               * press of 6 decrypted the message, and serial showed
               * "Challenge3 entered6" on that first press.
               *
               * The host cannot tell which mode the key is in - OKCONNECT
               * returns HW_MODEL(UNLOCKED) and a version string, and the
               * capabilities report (OKGETLABELS 'c') carries the modes the
               * BUILD supports, not the one this key is set to. Until it
               * carries the current setting, the honest line is one that is
               * true under both, and the digits stay because in challenge mode
               * the host is the only thing that can tell the user what they
               * are.
               *
               * Why it is worth the words: the digits are SHA256(ct), a
               * function of the payload, so "enter 6-2-5" reads as proof that
               * this specific message is what is being confirmed. In press
               * mode that binding does not exist and any press authorises
               * whatever is staged. Overstating it is the one direction this
               * message must not err in.
               *
               * The wording matches the age-derive pages' challenge box, which
               * already hedged correctly. */
              _api.emit("status", `Confirm on your OnlyKey (${secondsRemaining}s) - tap any button, or press ${pin.join('-')} if it asks for a challenge code.`);
      ...
      /* Clear the status so the next run starts from nothing.
       *
       * _$status() only assigns when handed a truthy value, so there was no
       * way back out of 'finished' - and msg_polling() sets 'finished' on ANY
       * ping error, not just on success. A failed operation therefore left the
       * page in the same state a successful one does, and the pages' click
       * handlers switch on exactly that: the next click took the 'finished'
       * branch and tried to copy the message to the clipboard instead of
       * retrying. Observed as "Oops, unable to copy message to clipboard, try
       * copying message manually" in answer to a click on Sign.
       *
       * packetnum is deliberately NOT reset here, for the reason given in
       * u2fSignBuffer(): the device keeps a high-water mark of opt3 across
       * operations and silently drops anything not strictly greater, so
       * restarting the count after an abort would lose the next operation's
       * first chunk. */
      function _$reset() {
        _status = undefined;
        _api.emit("_status", undefined);
      }
