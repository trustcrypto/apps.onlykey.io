            ...
            setup: function(app, $page, pathname, doInit) {
                ...
                if (!$("#action").data("changeSet")) {
                    ...
                    page.button.addEventListener('click', async function() {
                        ...
                        /* A failed run leaves _status at 'finished' -
                         * msg_polling() sets it on any ping error, not only on
                         * success - so this switch took the 'finished' branch
                         * and tried to copy the message to the clipboard
                         * instead of retrying. Clear it first when the last
                         * run errored, and this click starts a new one.
                         * The button's own class is the record of that: the
                         * "error" handler above sets it, "working" and "done"
                         * clear it. */
                        if (page.button.classList.contains('error')) {
                            page.okpgp._$reset();
                            page.button.classList.remove('error');
                        }
